...
\begin{itemize}
\item Exploring the electroweak and Higgs sectors measuring polarization in jets, observing Vector Boson Scattering (VBS) in semileptonic final states, providing a complete description of higher dimension EFT operators
\item Delivering the Outer Tracker for the Phase-2 upgrade and ensuring the collection of high quality data at HL-LHC
...
\end{equation}
where the operators $\mathcal{O}_\alpha^{(D)}$ are constructed with SM fields at some dimension $D_\alpha$, and $c_\alpha^{(D_\alpha)}$ are the Wilson coefficients of that effective source of new physics.
...
\textbf{Combination of dimension-6 and dimension-8 operators} Historically dimension-6 operators have been less studied. As a consequence, separate analyses search for dimension-8 and 6 in the same VBS $\ell\nu$qq final state. A natural extension of the current effort would be to combine these two analyses to obtain a more extended description of the EFT Lagrangian. To achieve this it will be necessary to involve the Fermilab theoretical group for a proper treatment of the interplay between the dimension-6 and 8 operators.
...
Additionally, the possibility to {\bf involve the Fermilab theoretical community will guarantee a correct implementation of the polarization for the hadronically decaying W and Z bosons, not yet fully achieved in simulation.} 
